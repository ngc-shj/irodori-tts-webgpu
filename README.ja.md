...
**デプロイ時はモデルをキャッシュすること。** 毎回 ~1.2GB（fp16）を取得するのは共有
ホスト（HF Spaces 等）では帯域の無駄です。`web/app.mjs` は localhost 以外のオリジンで
各モデルファイルを **Cache Storage API** に保存するので、再訪問者は一度きりの
ダウンロードで済みます（再エクスポート時は `CACHE_NAME` を更新）。自前ローダを書く
場合も同様にするか、`*.onnx`/`*.onnx.data` を `Cache-Control: public,
max-age=31536000, immutable` ＋内容ハッシュ名で配信してください。（localhost は
`web/serve.py` が `no-store` を返すので、再生成したアーティファクトが常に最新になります。）

**ブラウザ注意: デプロイ先では Chrome を使うこと。** 同じ M3 Pro で Chrome は localhost と
同速（~0.5× RTF）でしたが、Edge はホスト上ページで DiT サンプリングステップが ~2〜3倍
遅くなりました（decode は無影響・同一 GPU・同一モデル）。これは**ユーザー側で回避不能**で、
省電力トグル OFF・電源接続・`throttle-main-thread-to-60hz` フラグ無効でも変わりませんでした。
Edge でも localhost は正常で、デプロイ先オリジンだけが絞られます。Chrome を使ってください。
