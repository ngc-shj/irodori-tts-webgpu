...
**ブラウザ注意:** デプロイ先では **Chrome 推奨**。同じ M3 Pro で Chrome は localhost と
同速（~0.5× RTF）でしたが、Edge は効率モード/スロットリングにより、ホスト上ページで
DiT サンプリングステップが ~2〜3倍遅くなりました（decode は無影響・同一 GPU・同一モデル）。
Edge 利用時は効率モードを切ると改善する場合があります。
