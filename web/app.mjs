...
// Persist the model files (~1.2 GB at fp16) in the Cache Storage API so deployed
// visitors download the set once, not on every load — re-fetching 1+ GB per visit
// is rude on shared hosts (HF Spaces etc.). Skipped on localhost, where serve.py
// sends `no-store` so regenerated artifacts stay fresh. Bump CACHE_NAME when the
// artifacts change (or clear site data).
...
async function loadModels(s) {
  ...
  if (useModelCache) { await dropStaleModelCaches(); log(`model cache: on (${CACHE_NAME})`); }
  log(`loading models (${key})…`);
  const sessions = {};
